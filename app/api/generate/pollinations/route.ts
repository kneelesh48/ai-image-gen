...
async function fetchImageAsBase64(url: string): Promise<string> {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Failed to fetch image from Pollinations: ${response.status} ${response.statusText}. ${errorBody}`
    );
  }
...
export async function POST(request: Request) {
  try {
    ...
    console.log(
      `Generating image with Pollinations model: ${model}, prompt: "${prompt}"`
    );
    ...
    return NextResponse.json(
      { error: `Failed to generate image via Pollinations: ${errorMessage}` },
      { status: 500 }
    );
  }
}
