...
export async function POST(request: Request) {
  try {
    ...
    console.log(
      `Generating image with xAI model: ${model}, prompt: "${prompt}"`
    );
    ...
    if (response.data && response.data.length > 0) {
      ...
      console.error(
        "No image data found in xAI response or response format is unexpected:",
        response
      );
      return NextResponse.json(
        { error: "No image data received from API or invalid response format" },
        { status: 500 }
      );
    }
  } catch (error: unknown) {
